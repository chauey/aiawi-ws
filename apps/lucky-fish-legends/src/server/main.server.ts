...
import { setupPetSystem } from './petSystem';
...
changePetRemote.OnServerEvent.Connect((player, petType, price) => {
  ...
  const leaderstats = player.FindFirstChild('leaderstats') as
    | Folder
    | undefined;
  const coinsValue = leaderstats?.FindFirstChild('Coins') as
    | IntValue
    | undefined;
  if (!coinsValue || coinsValue.Value < actualPrice) {
    print(
      `❌ ${player.Name} can't afford ${petType} (needs ${actualPrice}, has ${coinsValue?.Value ?? 0})`,
    );
    return;
  }
...
function init() {
  ...
  // Setup advanced pet system with bonuses
  setupPetSystem();
